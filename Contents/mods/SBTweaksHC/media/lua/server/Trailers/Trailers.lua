-- ** ************************************************************* **
-- ** This file overwrites the Tsar Trailers functions              **
-- ** It must be called Trailers/Trailers.lua or it will not work   **
-- ** ************************************************************* **
-- MOD: TsarTrailers (2282429356)
-- CHANGES:
--    - Generators use GeneratorConsumption values instead of CarGasConsumption
--    - Generator consumption is 1/10th the amount of the GeneratorConsumption
...
-- ************************************************************ --
-- ** THIS IS THE FUNCTION THAT HANDLES THE FUEL CONSUMPTION ** --
-- ************************************************************ --
function Trailers.Update.GasTankFix(trailer, part, elapsedMinutes)
	...
	if elapsedMinutes > 0 and amount > 0 and trailer:isEngineRunning() then
		...
		-- ----------------------------------------------------------------------- --
		-- Change it to use the GeneratorConsumption values and make it 10x more efficient than tiny ones
		local newAmount = (speedMultiplier / gasMultiplier) * (SandboxVars.GeneratorFuelConsumption/10)
		-- ----------------------------------------------------------------------- --
		...
		if trailer:getModData()["generatorObject"] then
			...
			if elapsedMinutes > 0 and amount > 0 and trailer:isEngineRunning() then
				if trailer:getModData()["generatorObject"] then
					...
					-- trailer:getModData()["generatorObject"]:setFuel(100);
				end
			end
			if trailer:getModData()["generatorObject"] then
